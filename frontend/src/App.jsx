...
function App() {
  ...
  const processFaceAttendance = useCallback(async (face) => {
    ...
    try {
      ...
      const historyResponse = await fetch(`/api/attendance/${student_code}`);
  ...
  const detectAndTrackFaces = useCallback(async () => {
    if (
      ...
      const video = videoRef.current;
      const facesRaw = await detectionModel.estimateFaces(video, {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setUsername("");
      setPassword("");
      setActiveOverlay(nextAction);
      if (nextAction === "manage") fetchStudents();
    } catch (err) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setRegisterStatus(`Đăng ký thành công: ${data.student.name}`);
      setName("");
      setStudentCode("");
      setFile(null);
      setPreview(null);
    ...
    <div className="App">
      ...
      <div className="ui-container">
        <header className="App-header">
          <div className="logo">TRẠM ĐIỂM DANH</div>
          <nav>
            <button
